...
class MakeModule extends Command
{
    ...
    /**
     * @var \Jora\Modular\Support\ModuleRegistry
     */
    ...
    public function handle()
    {
        ...
        if (count($name) > 1) {
            $this->parent_path = $name[0];
            $this->module_name = $name[1];
        } else {
            $this->module_name = $name[0];
            $this->parent_path = null;
        }
        $this->class_name_prefix = Str::studly($this->module_name);
        ...
        if ($this->parent_path) {
            $this->base_path = $this->module_registry->getModulesPath() . '/' . $this->parent_path . '/' . $this->module_name;
        } else {
...
//            'StubTestCaseBase' => class_basename($tests_base),
        ...
        foreach ($this->getStubs() as $destination => $stub_file) {
            if ($destination === 'composer.json' && !empty($this->parent_path)) {
                $parent_path = $this->module_registry->getModulesPath() . '/' . $this->parent_path;
                $parent_composer_json = $parent_path . '/composer.json';
                if ($this->filesystem->exists($parent_composer_json)) {
                    $parent_composer = json_decode($this->filesystem->get($parent_composer_json), true, 512, JSON_THROW_ON_ERROR);
                    $this->updateParentComposer($parent_composer, $parent_composer_json);
                } else {
                    $this->createParentComposer($parent_composer_json);
                }
                continue;
            }
    ...
    protected function updateParentComposer(array &$parent_composer, string $parent_composer_json)
    {
        if (!isset($parent_composer['autoload']['psr-4']["{$this->module_namespace}\\{$this->class_name_prefix}\\"])) {
            $parent_composer['autoload']['psr-4']["{$this->module_namespace}\\{$this->class_name_prefix}\\"] = "{$this->module_name}/src";
            $this->filesystem->put($parent_composer_json, json_encode($parent_composer, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES));
            $this->line(" - Wrote to <info>{$parent_composer_json}</info>");
        }

        if (!isset($parent_composer['extra']['laravel']['providers'])) {
            $parent_composer['extra']['laravel']['providers'] = [];
        }

        $provider = "{$this->module_namespace}\\{$this->class_name_prefix}\\Providers\\{$this->class_name_prefix}ServiceProvider";
        if (!in_array($provider, $parent_composer['extra']['laravel']['providers'], true)) {
            $parent_composer['extra']['laravel']['providers'][] = $provider;
            $this->filesystem->put($parent_composer_json, json_encode($parent_composer, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES));
            $this->line(" - Wrote to <info>{$parent_composer_json}</info>");
        }
    }

    protected function createParentComposer(string $parent_composer_json)
    {
        $parent_composer = [
            'name' => $this->composer_namespace . '/' . $this->parent_path,
            'description' => $this->parent_path,
            'version' => '1.0.0',

            'autoload' => [
                'psr-4' => [
                    "{$this->module_namespace}\\{$this->class_name_prefix}\\" => "{$this->module_name}/src",
                ],
            ],
            ...
            'extra' => [
                'laravel' => [
                    'providers' => [
                        "{$this->module_namespace}\\{$this->class_name_prefix}\\Providers\\{$this->class_name_prefix}ServiceProvider",
                    ],
                ],
            ]
        ];
...
//// if exist parent path find composer.json and add require or not  exist composer.json create composer.json for example {
////  "name": "module/pensioner",
////  "description": "Global references module",
////  "version": "1.0.0",
////  "autoload": {
////    "psr-4": {
////    }
...
////  "authors": [
////    {
////      "name": "Jorabek Yusupov",
////      "email": "j_yusupov@uzinfocom.uz"
////    }
////  ],
////  "extra": {
////    "laravel": {
...
//            $parent_composer_json = $parent_path . '/composer.json';
//            if ($this->filesystem->exists($parent_composer_json)) {
//                $parent_json_file = new JsonFile($parent_composer_json);
//                $parent_definition = $parent_json_file->read();
//                if (!isset($parent_definition['require'][$this->composer_name])) {
//                    $this->line(" - Adding require statement for <info>{$this->composer_name}:*</info>");
//                    $has_changes = true;
//
//                    $parent_definition['require']["{$this->parent_path}/{$this->module_name}"] = '*';
//                    $parent_definition['require'] = $this->sortComposerPackages($parent_definition['require']);
//                    $parent_json_file->write($parent_definition);
//                    $this->line(" - Wrote to <info>{$parent_json_file->getPath()}</info>");
//                } else {
//                    $this->line(' - Nothing to update (require entry already exist)');
//                }
//            } else {
//                $this->line(" - Creating composer.json for <info>{$this->parent_path}</info>");
//                $parent_json_file = new JsonFile($parent_composer_json);
//                $parent_json_file->write([
//                    'name' => $this->composer_namespace . '/' . $this->parent_path,
//                    'version' => '1.0',
//                    "type" => "library",
//                    "license" => "proprietary",
...
//                            'type' => 'path',
//                            'url' => './*',
////                            'url' => str_replace('\\', '/', config('app-modules.modules_directory', 'app-modules')) . '/' . $this->parent_path . '/' . $this->module_name,
//                            'options' => [
//                                'symlink' => true,
//                            ],
//                        ],
...
//                    'description' => $this->parent_path,
//                    'require' => [
//                        "{$this->parent_path}/{$this->module_name}" => '*',
//                    ],
//                ]);
//                $this->line(" - Wrote to <info>{$parent_json_file->getPath()}</info>");
//            }
//            // check main composer.json and add require
//            if (!isset($definition['require'][$this->composer_namespace . '/' . $this->parent_path])) {
//                $this->line(" - Adding require statement for <info>{$this->composer_namespace}/{$this->parent_path}:*</info>");
//                $has_changes = true;
//
//                $definition['require']["{$this->composer_namespace}/{$this->parent_path}"] = '*';
//                $definition['require'] = $this->sortComposerPackages($definition['require']);
//            } else {
//                $this->line(' - Nothing to update (require entry already exist)');
//            }
//
//        } else if (!isset($definition['require'][$this->composer_name])) {
//            $this->line(" - Adding require statement for <info>{$this->composer_name}:*</info>");
//            $has_changes = true;
//
...
//        }
    ...
    protected function getStubs(): array
    {
        ...
        return [
            ...
            'config/StubConfig.php' => $this->pathToStub('config.php'),
            'lang/ru.json' => $this->pathToStub('lang.json'),
            'lang/en.json' => $this->pathToStub('lang.json'),
            'lang/uz.json' => $this->pathToStub('lang.json'),
            'lang/oz.json' => $this->pathToStub('lang.json'),
            'resources/views/index.blade.php' => $this->pathToStub('view.blade.php'),
            'routes/route.php' => $this->pathToStub('web-routes.php'),
            'src/Seeders/.gitkeep' => $this->pathToStub('.gitkeep'),
            'src/Filament/Resources/.gitkeep' => $this->pathToStub('.gitkeep'),
            'src/Filament/Pages/.gitkeep' => $this->pathToStub('.gitkeep'),
            'src/Factories/.gitkeep' => $this->pathToStub('.gitkeep'),
            'src/Models/.gitkeep' => $this->pathToStub('.gitkeep'),
            'src/Controllers/.gitkeep' => $this->pathToStub('.gitkeep'),
            'src/Repositories/.gitkeep' => $this->pathToStub('.gitkeep'),
            'src/Policies/.gitkeep' => $this->pathToStub('.gitkeep'),
            'src/Services/.gitkeep' => $this->pathToStub('.gitkeep'),
            'src/Providers/StubClassNamePrefixServiceProvider.php' => $this->pathToStub('ServiceProvider.php'),
            'migrations/.gitkeep' => $this->pathToStub('.gitkeep'),

//            'src/Livewire/StubClassNamePrefixTable.php' => $this->pathToStub('Table.php'),
//            'migrations/StubMigrationPrefix_StubModuleName_module.php' => $this->pathToStub('migration.php'),
...
//            'src/Models/StubClassNamePrefixTable.php' => $this->pathToStub('model.php'),
